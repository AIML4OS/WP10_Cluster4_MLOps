...
Model versions are stored locally. Each time a new model is (re-)trained, the hyperparameters, tokenizers, translation tables, and models are saved, together with the respective versioned R code used to train, evaluate and serve the model. Once deployed to the rsconnect server, we can switch between model versions, enabling the reproduction of past experiments.
